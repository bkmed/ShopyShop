...
export const suppliersDb = {
    ...
    add: async (supplier: Omit<Supplier, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
...
export const supplierProductsDb = {
    ...
    getPreferredSupplier: async (productId: string): Promise<SupplierProduct | null> => {
        return (
    ...
    update: async (id: string, updates: Partial<SupplierProduct>): Promise<void> => {
            ...
            if (updates.isPreferred && current.productId) {
                mockSupplierProducts = mockSupplierProducts.map(sp => {
                    if (
                        ...
                        return { ...sp, isPreferred: false, updatedAt: new Date().toISOString() };
                    }
                    return sp;
                });
            }
