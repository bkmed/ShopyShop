...
export const ReclamationDetailScreen = () => {
    ...
    const handleUpdateStatus = async (newStatus: Reclamation['status']) => {
        ...
        setReclamation(prev => (prev ? { ...prev, status: newStatus, updatedAt: new Date().toISOString() } : null));
    };

    const getStatusColor = (status: Reclamation['status']) => {
        switch (status) {
            case 'pending': return '#FFA500';
            case 'investigating': return '#2D5BFF';
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <View style={[styles.header, { backgroundColor: getStatusColor(reclamation.status) }]}>
                <Text style={styles.headerStatus}>{reclamation.status.toUpperCase()}</Text>
                <Text style={styles.headerDate}>{t('reclamations.submitted')}: {reclamation.createdAt}</Text>
            </View>
            ...
            <View style={styles.content}>
                ...
                <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                    ...
                    <Text style={[styles.orderIdText, { color: theme.colors.primary, fontWeight: 'bold' }]}>
                        #{reclamation.orderId}
                    </Text>
                </View>
                            ...
                            {['pending', 'investigating', 'resolved', 'rejected'].map((s) => (
                                <TouchableOpacity
                                    ...
                                    style={[
                                        styles.statusBtn,
                                        {
                                            backgroundColor: reclamation.status === s ? getStatusColor(s as any) : 'transparent',
                                ...
                                >
                                    <Text style={[
                                        ...
                                        { color: reclamation.status === s ? '#FFF' : getStatusColor(s as any) }
                                    ]}>
                                        {s.toUpperCase()}
                                    </Text>
                                </TouchableOpacity>
