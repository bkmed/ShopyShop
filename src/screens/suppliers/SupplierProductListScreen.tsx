...
export const SupplierProductListScreen = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [supplierData, supplierProductsData, allProducts] =
        await Promise.all([
      ...
      const enrichedProducts = supplierProductsData
        .map(sp => {
  ...
  const handleLinkProduct = async () => {
    if (!selectedProductId || !linkPrice) {
      Alert.alert(
        t('common.error'),
        t('common.requiredFields') || 'Product and Price are required',
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        t('common.success'),
        t('suppliers.productLinked') || 'Product linked successfully',
      );
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.priceRow}>
          ...
          {item.isPreferred && (
            ...
            >
              <Text
                style={[styles.preferredText, { color: theme.colors.primary }]}
              >
