...
export const PickPackDetailScreen = () => {
  ...
  const toggleItemStatus = async (
    ...
    try {
      ...
      if (allPicked && !order.pickedAt)
        updates.pickedAt = new Date().toISOString();
      if (allPacked && !order.packedAt)
      ...
      for (const item of order.items) {
        if (item.packed) {
          try {
            ...
            console.error(
              `Failed to deduct stock for ${item.productName}:`,
              stockError,
            );
        ...
        <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
          ...
          <View style={styles.row}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.value, { color: theme.colors.text }]}>
                {t(`pickPack.${order.priority}`).toUpperCase()}
              </Text>
            </View>
          </View>
        </View>
            ...
            <View style={styles.actionRow}>
              ...
              >
                <Text
                  style={[
                    ...
                    { color: item.picked ? '#10B981' : theme.colors.text },
                  ]}
                >
                  {item.picked
                    ? '✓ ' + t('pickPack.picked')
              ...
              >
                <Text
                  style={[
                    styles.actionBtnText,
                    { color: item.packed ? '#3B82F6' : theme.colors.text },
                  ]}
                >
                  {item.packed
                    ? '✓ ' + t('pickPack.packed')
