...
export const StockReceptionAddEditScreen = () => {
  ...
  const handleSave = async () => {
    ...
    if (!referenceNumber.trim())
      newErrors.referenceNumber = t('common.required');
    if (!selectedSupplier) newErrors.supplierId = t('common.required');
    if (items.length === 0)
      newErrors.items =
        t('stockReception.errorNoItems') || 'Please add at least one item';
    ...
    try {
      ...
      const receptionData = {
        ...
        totalCost: items.reduce(
          (acc, item) => acc + (item.unitPrice || 0) * item.expectedQuantity,
          0,
        ),
        currency: 'USD',
        notes,
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          >
            ...
            >
              {selectedSupplier
                ...
                : t('stockReception.selectSupplier') || 'Select Supplier'}
            </Text>
          </TouchableOpacity>
          {errors.supplierId && (
        ...
        <View style={styles.formGroup}>
          <RequiredLabel
            label={t('stockReception.expectedDate') || 'EXPECTED DATE'}
          />
          <TextInput
            style={[
              styles.input,
        ...
        {items.map(item => (
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.itemSku, { color: theme.colors.subText }]}>
                {item.sku}
              </Text>
              <Text style={[styles.itemQty, { color: theme.colors.text }]}>
                {t('stockReception.expectedQuantity')}: {item.expectedQuantity}
              </Text>
            </View>
      ...
      <Modal visible={productModalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          ...
          >
            <Text style={[styles.modalTitle, { color: theme.colors.text }]}>
              {t('stockReception.addProduct') || 'Add Product'}
            </Text>
            ...
            >
              {t('stockReception.expectedQuantity').toUpperCase()}
            </Text>
            <TextInput
              style={[
                styles.input,
