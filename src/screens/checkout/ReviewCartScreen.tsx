...
export const ReviewCartScreen = () => {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <GlassHeader title={t('checkout.review') || "REVIEW"} showBack onBackPress={() => navigation.goBack()} />

            <ScrollView contentContainerStyle={styles.scroll}>
                <Text style={[styles.sectionTitle, { color: theme.colors.subText }]}>SUMMARY</Text>
                    ...
                    {cartItems.map((item) => {
                        ...
                        return (
                            <View key={item.productId} style={styles.itemRow}>
                                {product.imageUris?.[0] ? (
                                    <Image source={{ uri: product.imageUris[0] }} style={styles.itemImage} />
                                ) : (
                                    <View style={[styles.itemImage, { backgroundColor: theme.colors.surface }]} />
                                )}
                                <View style={styles.itemInfo}>
                                    <Text style={[styles.itemName, { color: theme.colors.text }]} numberOfLines={1}>{product.name}</Text>
                                    <Text style={[styles.itemQty, { color: theme.colors.subText }]}>QTY: {item.quantity}</Text>
                ...
                <View style={[styles.infoSection, { borderTopColor: theme.colors.border }]}>
                    <View style={styles.infoRow}>
                        <Text style={[styles.label, { color: theme.colors.subText }]}>DELIVERY</Text>
                        <View style={{ alignItems: 'flex-end' }}>
                            <Text style={[styles.value, { color: theme.colors.text }]}>{deliveryMethod?.title}</Text>
                            <Text style={[styles.subValue, { color: theme.colors.subText }]}>{deliveryMethod?.time}</Text>
                        </View>
                    </View>
                    <View style={styles.infoRow}>
                        <Text style={[styles.label, { color: theme.colors.subText }]}>ADDRESS</Text>
                        <View style={{ alignItems: 'flex-end', flex: 1, marginLeft: 20 }}>
                            <Text style={[styles.value, { color: theme.colors.text }]}>{shippingAddress?.fullName}</Text>
                            <Text style={[styles.subValue, { color: theme.colors.subText, textAlign: 'right' }]} numberOfLines={2}>
                                {shippingAddress?.addressLine1}, {shippingAddress?.city}
                            </Text>
                        </View>
                    </View>
                    <View style={styles.infoRow}>
                        <Text style={[styles.label, { color: theme.colors.subText }]}>PAYMENT</Text>
                        <Text style={[styles.value, { color: theme.colors.text }]}>
                            {paymentMethod === 'card' ? 'Credit Card' : paymentMethod === 'cash' ? 'Cash on Delivery' : 'Apple Pay'}
                        </Text>
                    </View>
                </View>
                ...
                <View style={[styles.summaryBox, { borderTopColor: theme.colors.border }]}>
                    <View style={styles.summaryRow}>
                        <Text style={[styles.summaryLabel, { color: theme.colors.subText }]}>SUBTOTAL</Text>
                        <Text style={[styles.summaryValue, { color: theme.colors.text }]}>{formatPrice(subtotal)}</Text>
                    </View>
                    <View style={styles.summaryRow}>
                        <Text style={[styles.summaryLabel, { color: theme.colors.subText }]}>SHIPPING</Text>
