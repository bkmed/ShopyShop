...
export const AddressAddEditScreen = () => {
    ...
    const handleSave = async () => {
        if (!fullName.trim() || !addressLine1.trim() || !city.trim() || !state.trim() || !postalCode.trim() || !country.trim() || !phone.trim()) {
            AlertService.showError(toast, t('common.fillAllFields') || 'Please fill all required fields');
            return;
        }

        if (!user?.id) {
            AlertService.showError(toast, t('common.notLoggedIn') || 'You must be logged in');
            return;
        }
        ...
        try {
            ...
            if (isEditing) {
                ...
                AlertService.showSuccess(toast, t('checkout.addressUpdated') || 'Address updated successfully');
            } else {
                ...
                AlertService.showSuccess(toast, t('checkout.addressAdded') || 'Address added successfully');
            }

            navigation.goBack();
        } catch (error) {
            console.error('Error saving address:', error);
            AlertService.showError(toast, t('common.saveError') || 'Failed to save address');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <Text style={[styles.title, { color: theme.colors.text }]}>
                {isEditing ? t('checkout.editAddress') || 'Edit Address' : t('checkout.addAddress') || 'Add Address'}
            </Text>
            ...
            <View style={styles.form}>
                ...
                <TextInput
                    ...
                    placeholder={t('checkout.enterAddress') || 'Street address, P.O. box, company name'}
                    placeholderTextColor={theme.colors.subText}
                />
                ...
                <TextInput
                    ...
                    placeholder={t('checkout.apartment') || 'Apartment, suite, unit, building, floor, etc.'}
                    placeholderTextColor={theme.colors.subText}
                />
                ...
                >
                    <Text style={styles.saveButtonText}>
                        {loading ? t('common.saving') || 'Saving...' : t('common.save') || 'Save Address'}
                    </Text>
                </TouchableOpacity>
            </View>
        </ScrollView>
