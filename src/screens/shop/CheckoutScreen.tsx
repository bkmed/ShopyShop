...
export const CheckoutScreen = () => {
    ...
    const handleApplyPromo = async () => {
        ...
        try {
            ...
            if (!promoObj) {
                Alert.alert(t('common.error'), t('cart.invalidPromo') || 'Invalid Promo Code');
                return;
            }

            const percentage = await promosDb.validatePromo(promoCode, 'all'); // 'all' check first?
            // Actually we need to check per item.

            for (const item of cartItems) {
                const p = products[item.productId];
                if (p) {
                    const itemRelevance = await promosDb.validatePromo(promoCode, p.categoryId);
                    if (itemRelevance > 0) {
            ...
            if (totalDiscount > 0) {
                ...
                Alert.alert(t('common.success'), `${t('cart.discountApplied') || 'Discount Applied'}: ${formatPrice(totalDiscount)}`);
            } else {
                Alert.alert(t('common.info'), t('cart.promoNotApplicable') || 'Promo code not applicable to these items');
                setDiscount(0);
            }
    ...
    const handlePlaceOrder = async () => {
        if (!address.trim()) {
            Alert.alert(t('common.error'), t('cart.addressRequired') || 'Address is required');
            return;
        }
        ...
        try {
            ...
            dispatch(clearCart());
            Alert.alert(t('common.success'), t('cart.orderPlaced') || 'Order placed successfully!', [
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>{t('cart.shippingAddress') || 'Shipping Address'}</Text>
                <TextInput
            ...
            <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>{t('cart.paymentMethod') || 'Payment Method'}</Text>
                    ...
                    {['card', 'cash'].map(method => (
                        <TouchableOpacity
                            ...
                            style={[
                                ...
                                paymentMethod === method && { borderColor: theme.colors.primary, backgroundColor: theme.colors.primary + '10' }
                            ]}
                            onPress={() => setPaymentMethod(method)}
                        >
                            <Text style={{ fontSize: 24 }}>{method === 'card' ? '💳' : '💵'}</Text>
            ...
            <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>{t('cart.promoCode') || 'Promo Code'}</Text>
            ...
            <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                <View style={styles.row}>
                    <Text style={[styles.label, { color: theme.colors.subText }]}>{t('cart.subtotal')}</Text>
                    <Text style={[styles.value, { color: theme.colors.text }]}>{formatPrice(subtotal)}</Text>
                </View>
                {discount > 0 && (
                    <View style={styles.row}>
                        <Text style={[styles.label, { color: '#4CAF50' }]}>{t('cart.discount') || 'Discount'}</Text>
                        <Text style={[styles.value, { color: '#4CAF50' }]}>-{formatPrice(discount)}</Text>
                    </View>
                )}
            ...
            >
                {processing ? (
                    ...
                    <Text style={styles.checkoutBtnText}>{t('cart.placeOrder') || 'Place Order'}</Text>
                )}
            </TouchableOpacity>
