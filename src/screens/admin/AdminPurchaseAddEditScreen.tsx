...
export const AdminPurchaseAddEditScreen = () => {
    ...
    const validate = () => {
        ...
        else if (isNaN(parseFloat(totalAmount))) newErrors.totalAmount = t('common.invalidAmount') || 'Invalid amount';
    ...
    const loadPurchase = async () => {
        try {
            ...
            if (purchase) {
                ...
                Alert.alert(t('common.error'), t('common.notFound') || 'Purchase not found');
                navigation.goBack();
            }
        } catch (error) {
            console.error('Error loading purchase:', error);
            Alert.alert(t('common.error'), t('common.loadFailed') || 'Failed to load purchase');
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEditing) {
                ...
                Alert.alert(t('common.success'), t('common.saved') || 'Saved successfully');
            } else {
                ...
                Alert.alert(t('common.success'), t('common.added') || 'Added successfully');
            }
            navigation.goBack();
        } catch (error) {
    ...
    const StatusButton = ({ value, label, color }: { value: Purchase['status'], label: string, color: string }) => (
        <TouchableOpacity
            style={[
                styles.statusButton,
                {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <GlassHeader
                title={isEditing ? (t('adminPurchases.edit') || 'Edit Purchase') : (t('adminPurchases.add') || 'Add Purchase')}
                showBack
            />
            ...
            <ScrollView contentContainerStyle={styles.content}>
                <View style={styles.formGroup}>
                    ...
                    />
                    {errors.orderId && <Text style={styles.errorText}>{errors.orderId}</Text>}
                </View>
                ...
                <View style={styles.row}>
                    <View style={[styles.formGroup, { flex: 1, marginRight: 8 }]}>
                        ...
                        <TextInput
                            style={[styles.input, {
                                backgroundColor: theme.colors.surface,
                                color: theme.colors.text,
                                borderColor: errors.totalAmount ? '#EF4444' : theme.colors.border
                            }]}
                            value={totalAmount}
                            onChangeText={(text) => {
                                setTotalAmount(text);
                    ...
                    <View style={[styles.formGroup, { width: 100 }]}>
                        <Text style={[styles.label, { color: theme.colors.text }]}>{t('common.currency') || 'Curr'}</Text>
                        <TextInput
                            style={[styles.input, {
                                backgroundColor: theme.colors.surface,
                                color: theme.colors.text,
                ...
                <View style={styles.formGroup}>
                    <Text style={[styles.label, { color: theme.colors.text }]}>{t('common.status') || 'Status'}</Text>
                    <View style={styles.statusGrid}>
                        <StatusButton value="pending" label={t('orders.status.pending')} color="#F59E0B" />
                        <StatusButton value="processing" label={t('orders.status.processing')} color="#3B82F6" />
                        <StatusButton value="shipped" label={t('orders.status.shipped')} color="#8B5CF6" />
                        <StatusButton value="delivered" label={t('orders.status.delivered')} color="#10B981" />
                        <StatusButton value="cancelled" label={t('orders.status.cancelled')} color="#EF4444" />
                    </View>
                </View>
