    ...
    const handlSave = async () => {
        ...
        try {
            ...
            if (isEditing) {
                ...
                Alert.alert(t('common.success'), t('users.createdSuccessfully') || 'User created');
            }
            navigation.goBack();
        } catch (error) {
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
            <View style={[styles.formContainer, { backgroundColor: theme.colors.surface }]}>
                <Text style={[styles.label, { color: theme.colors.text }]}>{t('profile.firstName') || 'Name'}</Text>
                <TextInput
                ...
                <Text style={[styles.label, { color: theme.colors.text }]}>{t('profile.email') || 'Email'}</Text>
                <TextInput
                    style={[styles.input, { color: theme.colors.text, borderColor: theme.colors.border }]}
                    value={email}
                ...
                <Text style={[styles.label, { color: theme.colors.text }]}>{t('roles.title') || 'Role'}</Text>
                <View style={styles.roleContainer}>
                    {roles.map((r) => (
                        ...
                        >
                            <Text style={[
                                styles.roleText,
                                role === r ? { color: '#FFF' } : { color: theme.colors.primary }
                            ]}>
                                {t(`roles.${r}`) || r}
                            </Text>
                        </TouchableOpacity>
